...
import { 
  ...
  Building2, ShieldCheck, Check, Zap, Award, Bell, History, X, Trash2, Eye
} from 'lucide-react';
import { gsheetService } from '../services/gsheet';
import { storageService } from '../services/storage';

const SPREADSHEET_ID = "15YqJid8tL_AAOROFDJ-D2H4ozQUlJUJxACrgOu2NUnQ";
const FILTER_ULTG = "BEKASI";

const SHEETS = [
  { name: "Anomali DS", title: "Anomali DS", gid: "0", type: "detail", category: "Anomali" },
  { name: "Pemasangan WAP/Corbuzier", title: "Pemasangan WAP/Corbuzier dan Cover Lugs Trafo", gid: "1158313206", type: "detail", category: "Keandalan" },
  { name: "Pemasangan Jaring", title: "Pemasangan Jaring (Kawat Harmonika)", gid: "220192563", type: "detail", category: "Keandalan" },
  { name: "Penambahan Media Isolasi", title: "Penambahan Media Isolasi pada Relay Mekanik Trafo", gid: "427933333", type: "detail", category: "Keandalan" }
];
...
// Komponen perender tombol/badge dokumen yang interaktif dan memanggil lightbox
function DocumentCell({ row, onImageClick }) {
  let imageUrl = null;
  const urlRegex = /https?:\/\/[^\s]+/i;
  
  if (row.originalRow) {
    for (let i = row.originalRow.length - 1; i >= 0; i--) {
      const cell = String(row.originalRow[i] || "").trim();
      const match = cell.match(urlRegex);
      if (match) {
        let rawUrl = match[0];
        const driveIdMatch = rawUrl.match(/\/file\/d\/([a-zA-Z0-9_-]+)/) || rawUrl.match(/id=([a-zA-Z0-9_-]+)/);
        if (driveIdMatch) {
  ...
  if (imageUrl) {
    return (
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#E0F5FF'; e.currentTarget.style.color = '#0078AE'; e.currentTarget.style.borderColor = '#BAE6FD'; }}
        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#F1F5F9'; e.currentTarget.style.color = '#475569'; e.currentTarget.style.borderColor = '#E2E8F0'; }}
      >
  ...
  return <span style={{ color: '#94A3B8', fontWeight: 600, fontSize: '1.2rem' }}>-</span>;
}
...
export default function HargiProgramKerja() {
  ...
  // Tab kategori ('ALL', 'ABO', 'KEANDALAN')
    ...
    try {
      const results = await Promise.all(
        SHEETS.map(async (sheet) => {
          try {
            const { rows, fromCache, lastUpdated: sheetLastUpdated } = await gsheetService.fetchSheetData(sheet.gid, forceRefresh, SPREADSHEET_ID);
            const rawRows = trimSheetRows(rows || []);

            const headerInfo = findHeader(rawRows, "GARDU INDUK");
            if (!headerInfo) {
              return {
                ...sheet,
                status: "empty",
            ...
            const matchEntries = rawRows
              ...
              .filter(({ row }) => {
                ...
                // Karena ini sheet HARGI (tidak ada kolom ULTG), tampilkan semua baris yang Gardu Induknya terisi
            ...
            return {
              ...
              displayRows: matches,
              records,
              message: matches.length ? `Terbaca ${matches.length} baris data` : "Tidak ada baris data",
  ...
  // Metrik dihitung EKSKLUSIF dari 4 program HARGI
  ...
  // Filter programs based on tab & search (Tampilkan SEMUA program meski target ULTG Bekasi = 0)
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'auto', fontSize: '0.78rem' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#F8FAFC', borderBottom: '2px solid #E2E8F0', textAlign: 'left', color: '#475569', fontWeight: 700 }}>
                        ...
                        {(() => {
                           const skipCols = ["NO", "LINK BA", "DOKUMENTASI", "DOKUMEN", "BERITA ACARA", "EVIDENCE"];
                           return (item.sheet.header || []).map((col, idx) => {
                             if (!col || skipCols.includes(String(col).toUpperCase())) return null;
                             return <th key={idx} style={{ padding: '10px 8px' }}>{col}</th>;
                           });
                        })()}
                        <th style={{ padding: '10px 8px', width: '13%', textAlign: 'center' }}>Dokumentasi</th>
                      </tr>
                    </thead>
                    <tbody>
                      {item.records.map((row, idx) => {
                         ...
                         return (
                          <tr key={`${row.gid}-${idx}`} style={{ borderBottom: '1px solid #F1F5F9', backgroundColor: idx % 2 === 0 ? '#FFFFFF' : '#FAFCFF' }}>
                            ...
                            {(item.sheet.header || []).map((col, cIdx) => {
                               if (!col || skipCols.includes(String(col).toUpperCase())) return null;
                               let val = row.originalRow ? row.originalRow[cIdx] : '-';
                               if (val === undefined || val === null || val === '') val = '-';
                               
                               // Bold styling for Gardu Induk
                               if (String(col).toUpperCase() === 'GARDU INDUK') {
                                 return (
                                   <td key={cIdx} style={{ padding: '8px 8px', fontWeight: 800, color: '#0078AE', wordBreak: 'break-word' }}>
                                     <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                                       <Building2 style={{ width: 14, height: 14, flexShrink: 0 }} /> {val}
                                     </span>
                                   </td>
                                 );
                               }
                               return <td key={cIdx} style={{ padding: '8px 8px', color: '#1E293B', wordBreak: 'break-word' }}>{val}</td>;
                            })}
                            <td style={{ padding: '8px 8px', textAlign: 'center' }}>
                              <DocumentCell row={row} onImageClick={setSelectedImage} />
                            </td>
                          </tr>
                         );
                      })}
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '24px', maxWidth: '1360px', margin: '0 auto' }}>
      ...
      <div className="pk-card" style={{ padding: '14px 18px', marginBottom: '20px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-end', gap: '14px' }}>
        {/* Search Input */}
        <div style={{ position: 'relative', flex: '1 1 300px', maxWidth: '400px' }}>
          ...
          <input
            ...
            style={{ paddingLeft: '34px', fontSize: '0.8rem', padding: '8px 12px 8px 34px', width: '100%', borderRadius: '8px', border: '1px solid #CBD5E1', outline: 'none' }}
          />
        </div>
      </div>
      ...
      {selectedImage && (
        ...
        >
          <div style={{ position: 'relative', maxWidth: '90%', maxHeight: '90%' }} onClick={(e) => e.stopPropagation()}>
            ...
            >
              Close <X style={{ width: 24, height: 24 }} />
            </button>
            <img 
              src={selectedImage} 

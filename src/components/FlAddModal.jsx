...
export default function FlAddModal({ isOpen, onClose, onAddSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.detail || data.message || 'Gagal menambahkan peralatan FL');
      }

      onAddSuccess(data.fl_devices || []);
      ...
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: '16px', width: '100%', maxWidth: '500px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', borderBottom: '1px solid #E2E8F0', backgroundColor: '#F8FAFC' }}>
          <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800, color: '#0F172A', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Server size={20} color="#EAB308" /> Tambah Peralatan FL
          </h3>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '20px' }}>
          ...
          <button 
            type="submit"
            disabled={isSubmitting}
            style={{ width: '100%', padding: '12px', backgroundColor: '#EAB308', color: '#FFFFFF', border: 'none', borderRadius: '8px', fontWeight: 800, fontSize: '1rem', cursor: isSubmitting ? 'not-allowed' : 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}
          >
